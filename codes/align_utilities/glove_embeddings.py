...
GLOVEURL = "http://nlp.stanford.edu/data/" # os.environ.get("GLOVEURL")
...
class GloveTransformer:
    ...
    def get_tokens(self, input_values: Iterable[str]) -> Set[str]:
        ...
        for value in input_values:
            ...
            for shingle in self.shingles(value):
    ...
    def shingles(self, value: str) -> Iterable[str]:
        """
    ...
    def ComputeSemanticScore(self, query_list, data_lake_list):
        query_embedding = self.transform(query_list).reshape(1,-1)
        data_lake_embedding = self.transform(data_lake_list).reshape(1,-1)
        if np.any(query_embedding) == True and np.any(data_lake_embedding) == True:
            sem_score_joint = cosine_similarity(query_embedding, data_lake_embedding)[0][0]
        else:
            sem_score_joint = 0.0
        # throw the intersecting items and compute similarity score
        intersection_items = set(query_list).intersection(data_lake_list)
        query_list = [item for item in query_list if item not in intersection_items]
        data_lake_list = [item for item in data_lake_list if item not in intersection_items]
        if len(query_list) == 0 or len(data_lake_list) == 0:
            sem_score_disjoint = 0.0
        else:
            query_embedding = self.transform(query_list).reshape(1,-1)
            data_lake_embedding = self.transform(data_lake_list).reshape(1,-1)
            if np.any(query_embedding) == True and np.any(data_lake_embedding) == True:
                sem_score_disjoint = cosine_similarity(query_embedding, data_lake_embedding)[0][0]
            else:
                sem_score_disjoint = 0.0
        return sem_score_joint, sem_score_disjoint
